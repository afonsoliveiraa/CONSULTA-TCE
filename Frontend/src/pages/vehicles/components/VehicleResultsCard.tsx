...
import type { VehicleColumnDefinition, VehicleColumnId } from "../vehicleQuery.types";
import { formatVehicleValue } from "../vehicleQuery.utils";
...
interface VehicleResultsCardProps {
  ...
  totalItems: number;
  totalPages: number;
  sortColumnId: VehicleColumnId;
  sortDirection: SortDirection;
  visibleColumns: VehicleColumnDefinition[];
  dropTargetColumnId: VehicleColumnId | null;
  onQuickSearchChange: (value: string) => void;
  onShowColumnModal: (value: boolean) => void;
  onPageChange: (page: number) => void;
  onSortChange: (columnId: VehicleColumnId) => void;
  onDragStart: (columnId: VehicleColumnId) => void;
  onDragOver: (event: DragEvent, columnId: VehicleColumnId) => void;
  onDragLeave: (columnId: VehicleColumnId) => void;
  onDrop: (columnId: VehicleColumnId) => void;
  onDragEnd: () => void;
  onExportCsv: () => void;
}

export const VehicleResultsCard: FunctionalComponent<VehicleResultsCardProps> = ({
  vehicles = [],
  filteredVehicles = [],
  carregandoConsulta,
  quickSearch,
        ...
        <tbody>
          {filteredVehicles.length > 0 ? (
            filteredVehicles.map((vehicle) => (
              <tr key={vehicle.id}>
                {visibleColumns.map((column) => (
                  <td key={column.id}>{formatVehicleValue(vehicle, column.id)}</td>
                ))}
              </tr>
            ))
          ) : (
            <tr>
              <td class="contracts-table__empty-cell" colSpan={visibleColumns.length || 1}>
                {carregandoConsulta
                  ? "Carregando dados..."
                  : vehicles.length === 0
                    ? "Pesquise uma placa ou RENAVAM para carregar os dados na grade."
                    : "Nenhum veiculo corresponde aos filtros aplicados."}
              </td>
            </tr>
          )}
        </tbody>
